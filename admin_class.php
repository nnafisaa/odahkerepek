...
Class Action {
	...
	function login(){
		...
		// $qry = $this->db->query("SELECT * FROM users where username = 'mmm' and password = '123'");
		if($qry->num_rows > 0){
			echo "<script>console.log('data lat sini', ".json_encode($qry->num_rows).")</script>";
			...
			echo "<script>console.log('is user exists', ".json_encode($qry->num_rows).")</script>";
			return 3;
		}
	}
